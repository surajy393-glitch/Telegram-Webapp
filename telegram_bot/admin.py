...
def user_info(tg_id: int) -> Optional[dict]:
    row = q_one("""SELECT tg_user_id, gender, age, country, city, language,
                          is_premium, coins, games_played, strikes
...
def reset_user_metrics(tg_id: int) -> None:
    ...
    q_exec("""
        ...
        SET gender=NULL, age=NULL, country=NULL, city=NULL,
            bio=NULL, photo_file_id=NULL, feed_username=NULL,
            feed_is_public=FALSE, feed_photo=NULL,
            search_pref='any',
            privacy_consent=FALSE,
            privacy_consent_date=NULL,
            age_verified=FALSE,
            age_agreement_date=NULL,
            coins=0, games_played=0, strikes=0,
            is_premium=%s
        WHERE tg_user_id=%s;
    """, (premium_flag, tg_id))
    ...
    # wipe ratings and reports logs (if tables exist)
    try:
